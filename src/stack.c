...
{
    if(stack != NULL) {
        upo_stack_node_t *node = malloc(sizeof(upo_stack_node_t)); // node is allocated 
        if(node == NULL) { // if allocation is not successful, operation is aborted
            perror("Unable to allocate node.\n");
            abort();
        }
        node -> data = data; // data field points to data
        node -> next = stack -> top; // next field points to the top of the stack
        stack -> top = node; // node is top of the stack
        stack -> size += 1; // stack size is increased
    }
}
...
void upo_stack_pop(upo_stack_t stack, int destroy_data)
{
    if(!upo_stack_is_empty(stack)) {
        upo_stack_node_t *node = stack -> top; // node points to the top of the stack
        stack -> top = stack -> top -> next; // top of the stack points to the next node
        stack -> size -= 1; // stack size is decreased
        if(destroy_data == 1) free(node -> data); // data is destroyed if flag is set
        free(node); // node is destroyed
    }
}
